...
class TextRedirector:
    ...
    def write(self, text):
        ...
        self.widget.see(END)  # automatisch nach unten scrollen
    def flush(self):
        pass
...
def CLEAR_CMD():
    txt_output.delete(1.0, END)  # alles in der Textbox löschen
...
def get_nums(index):
    ...
    if index == 0:  # Einzelzahl
        ...
        try:
            ...
            PLOTT_EIGENSCHAFTEN_FOLGE(folge)
            PLOTT_LISTE(folge)
        except:
        ...
        try:
            ...
            #print(zahlen_liste)
            folge = BERECHNEFOLGE(zahlen_liste)
            PLOTT_EIGENSCHAFTEN_BEREICH(folge)
            PLOTT_BEREICH(folge)
        except:
            print("Ungültiger Bereich!")

def PLOTT_EIGENSCHAFTEN(folge):
    startwert = folge[0]
    min_wert = min(folge)
    max_wert = max(folge)
    stoppzeit = len(folge) - 1  # Anzahl der Schritte bis 1

    # größte Fluktuation zwischen zwei aufeinanderfolgenden Zahlen
    fluktuation = 0
    fluktuation_index = 0
    for i in range(len(folge)-1):
        diff = abs(folge[i+1] - folge[i])
        if diff > fluktuation:
            fluktuation = diff
            fluktuation_index = i  # der Schritt, von dem die größte Änderung ausgeht

    print(f"Fluktuation (größter Schritt): {fluktuation} "
          f"(von Schritt {fluktuation_index}: {folge[fluktuation_index]} → {folge[fluktuation_index+1]})")
    print(f"Startwert: {startwert}")
    print(f"Min Wert: {min_wert}")
    print(f"Max Wert: {max_wert}")
    print(f"Stoppzeit (Anzahl Schritte): {stoppzeit}")
...
def PLOTT_EIGENSCHAFTEN_FOLGE(folgen_dict):
    """
    Gibt die Eigenschaften jeder Collatz-Folge in 'folgen_dict' aus:
    - Startwert
    - Minimalwert
    - Maximalwert
    - Stoppzeit
    ...
    for startwert, folge in folgen_dict.items():
        min_wert = min(folge)
        max_wert = max(folge)
        stoppzeit = len(folge) - 1  # Schritte bis 1

        # größte Fluktuation zwischen aufeinanderfolgenden Zahlen
        fluktuation = 0
        fluktuation_index = 0
        for i in range(len(folge)-1):
            diff = abs(folge[i+1] - folge[i])
        ...
        print(f"--- Collatz-Folge für {startwert} ---")
        print(f"Fluktuation (größter Schritt): {fluktuation} "
              f"(von Schritt {fluktuation_index}: {folge[fluktuation_index]} → {folge[fluktuation_index+1]})")
        print(f"Startwert: {startwert}")
        print(f"Min Wert: {min_wert}")
...
def PLOTT_EIGENSCHAFTEN_BEREICH(folgen_dict):
    """
    Gibt die Eigenschaften aller Collatz-Folgen eines Bereichs aus:
    - Startwert
    - Minimalwert
    - Maximalwert
    - Stoppzeit
    ...
    """
    if not folgen_dict:
        print("Keine Zahlen im Bereich!")
        return

    for startwert, folge in folgen_dict.items():
        if len(folge) == 1:  # nur eine Zahl -> keine Fluktuation
            min_wert = max_wert = startwert
            stoppzeit = 0
            fluktuation = 0
            fluktuation_index = 0
        else:
            min_wert = min(folge)
            max_wert = max(folge)
            stoppzeit = len(folge) - 1
            fluktuation = max(abs(folge[i+1] - folge[i]) for i in range(len(folge)-1))
            fluktuation_index = next(i for i in range(len(folge)-1)
                                     if abs(folge[i+1] - folge[i]) == fluktuation)
        ...
        print(f"Stoppzeit (Anzahl Schritte): {stoppzeit}")
        print(f"Fluktuation (größter Schritt): {fluktuation} "
              f"(von Schritt {fluktuation_index}: {folge[fluktuation_index]} → {folge[-1]})")
        print("-" * 40)
...
# === Textbox für Ausgaben mit Scrollbar ===
frame_output = Frame(window, bg="#1e1e2f")
frame_output.place(x=50, y=300, width=900, height=350)

scrollbar = Scrollbar(frame_output)
scrollbar.pack(side=RIGHT, fill=Y)

txt_output = Text(frame_output, wrap=WORD, yscrollcommand=scrollbar.set,
                  font=("Consolas", 12), bg="#2e2e3f", fg="#f0f0f0")
txt_output.pack(fill=BOTH, expand=True)

scrollbar.config(command=txt_output.yview)
